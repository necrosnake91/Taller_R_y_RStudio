...
knitr::include_graphics("./rstudiolayout.jpeg")
```
...
# Ejercicio 2
1. Declara una variable que se llame "rios" nueva que contenga un vector con los siguientes datos: Balsas, Usumacinta, Grijalva, Lerma, Balsas, Bravo, Grijalva, Balsas, Lerma y Bravo
...
```{r}
#Obten los índices de los elementos de la columna 1 que sean distintos de tres usando la función which
which(mi_matriz[, 1] != 3)
#Obten los índices de los elementos de la fila 1 que sean menores a 9
which(mi_matriz[1, ] < 9)
...
Adicionalmente, los nombres de las columnas de un data frame se obtienen con la función `names()`. Recuerda que esta función solamente es **aplicable exclusivamente** a los data frames:
```{r}
...
Sin embargo, para evitar usar la función `which()` y confundirnos con la sintaxis, existe la función `subset()` que nos permitirá extraer datos de un data frame de una manera más amigable:
```{r}
#Extrae los elementos que sean menores a 3 de la columna 3
...
En ocasiones requerimos adicionar nuevas columnas cuyos valores dependen de una operación en otra columna. Para adicionar nuevas columnas al data frame tendrás que asignar la nueva columna usando el símbolo de $ y escribiendo el nombre:
```{r}
...
Habitualmente, las tablas de datos se generan en un procesador de textos distinto a R y para analizarlos requerimos importar los datos al ambiente de R. Cuando generes tablas hazlo desde excel o empleando un **procesador de texto plano**. Al generar las tablas ten en mente lo siguiente:

- Evita usar nombres para las columnas con espacios en blanco o caracteres especiales (acentos, ñ, diérsis...)

- Evita dejar celdas en blanco. Si no tienes valores, llena la celda con **NA**
...
sep: Es el tipo de separador de datos que se emplea. Si los datos están separados por tabuladores usa `\t`, si están separados por comas usa `,`
